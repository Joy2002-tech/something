(() => {
  ...
  const safeUrl = (v) => { try { const u = new URL(String(v), location.origin); return u.protocol === 'https:' ? u.href : ''; } catch { return ''; } };
  ...
  const sourceLink = (item) => { const url = safeUrl(item?.sourceUrl); return url ? `<a href="${esc(url)}" target="_blank" rel="noopener noreferrer">Source ↗</a>` : ''; };
  ...
  function ipoRow(item) {
    ...
    return `<tr>
      <td><strong>${esc(fmt(item.company))}</strong><small>${esc(fmt(item.exchange, ''))}${item.category ? ` · ${esc(item.category)}` : ''}</small></td>
      <td>${esc(fmt(item.dates))}</td>
      <td>${esc(fmt(item.priceBand || (item.issuePrice ? money(item.issuePrice) : '-')))}</td>
      <td>${esc(fmt(item.issueSize))}</td>
      <td><strong>${gmp}</strong><small>${item.gmp !== undefined && item.gmp !== null ? gmpPct : 'Unofficial / unavailable'}</small></td>
      <td>${esc(fmt(item.subscription))}</td>
  ...
  function renderPerformance(rows) {
    ...
    body.innerHTML = (rows || []).map(item => `<tr>
      <td><strong>${esc(fmt(item.company))}</strong></td>
  ...
  function renderUnlisted(rows) {
    ...
    grid.innerHTML = (rows || []).map(item => `<article class="unlisted-live-card">
      <div class="live-top"><small>UNLISTED</small><span>${esc(fmt(item.status))}</span></div>
      <h3>${esc(fmt(item.company))}</h3>
      <div class="live-price">${money(item.referencePrice)}</div>
      <p>${esc(fmt(item.note, 'Reference / indicative price only. Verify transaction terms before acting.'))}</p>
